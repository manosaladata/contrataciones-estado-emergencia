...
mod_mapa_mon_uii<-function(id) {leafletOutput(NS(id,"map_mon"))}
# mod_mapa_mon_ui <- function(id){
#   ns <- NS(id)
#   tagList(
#  
#   )
...
#' mapa_mon Server Function
#'
#' @noRd 


mod_maps_module_server<-function(id){
  moduleServer(id, function(input, output, session) {
    output$map_mon <- renderLeaflet({map_mon})})}

# mod_mapa_mon_server <- function(input, output, session){
#   ns <- session$ns
#  
# }
    
## To be copied in the UI
# mod_mapa_mon_ui("mapa_mon_ui_1")
    
## To be copied in the server
# callModule(mod_mapa_mon_server, "mapa_mon_ui_1")

...
data<-as.data.frame(proveedores2_num[1:200,])
...
trabajo<-function(ruc){
  ...
  trabajadores_agosto<-empleados[["2020-10"]][["trabajadores"]]
}}
...
###############TRABAJADORES###
##############################
#Se usará solo para un ranking de 500, entonces solo corremos hasta el 3
...
list2env(lapply(mget(paste0("trabajadores", 1)), function(x) {   #Solo necesitamos 200 (para más 1:n)
  x[sapply(x, is.null)] <- "ND-ANUAL"
  x}), 
  .GlobalEnv)

#COMBINAMOS LISTAS
Trabajadores_list<-c(trabajadores1)
                     #, trabajadores2, trabajadores3) 
         #             ,trabajadores4, trabajadores5,
...
fechas_sum<-c(fecha_ins1)
              #,fecha_ins2,fecha_ins3)
              # ,fecha_ins4,fecha_ins5,
...
total<-cbind(data,Trabajadores,fechas)
#total
total<-total[,c(1,2,6,5,3,4)]   #Reordenamos
#names(total)
#CAMBIAMOS EL NE POR ND-MES (NO DECLARACIÓN EN EL ÚLTIMO MES DE ANÁLISIS)
names(total)<-c("Proveedor","RUCProveedor","Fecha de Inscripción", "Número de Trabajadores(en Octubre)","Moto Adjudicado (millones de soles).","Contratos")
#names(total)
...
#GUARDAMOS NUEVA DATA:NO! TRABAJAR CON WORKSPACE
#write.table(x=total, file="DATOS_api_oct", sep= ";",row.names=FALSE) #Num.Trab en octubre.
